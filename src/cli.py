...
def analyze(date):
    ...
    if not date:
        ...
        click.echo(f"  Total negative units: {total_negative_units}")
        click.echo(f"  Total negative hours: {total_negative_hours:.2f}")
    else:
        ...
        click.echo(f"\nAnalysis for {date}:")
        click.echo(f"  Resolution: {result['resolution']}")
        click.echo(f"  Negative units: {result['negative_units']}")
...
def calculate(installation_date, start_date, end_date):
    ...
    click.echo(f"  Negative units: {result['total_negative_units']}")
